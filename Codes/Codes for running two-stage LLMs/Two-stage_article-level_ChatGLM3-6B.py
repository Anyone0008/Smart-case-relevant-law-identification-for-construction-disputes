...
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/Two-stage_article-level_ChatGLM3-6B", trust_remote_code=True)
model = AutoModel.from_pretrained("/root/autodl-tmp/Two-stage_article-level_ChatGLM3-6B",  trust_remote_code=True,top_p=0, 
  temperature=0).cuda()
...
years=["Two-stage_act-level_ChatGLM3-6B"]
for year in years:
  print("Year",year)
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_Two-stage_article-level_ChatGLM3-6B:",answer1.strip().replace("\n",""))

 
    df = pd.concat([df, df2], axis=0)

  save_df_to_excel(df, "Two-stage_article-level_ChatGLM3-6B.xlsx", "sheet1")
