...
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/One-stage_Baichuan2-13B-Chat",
    revision="v2.0",
    use_fast=False,
    trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("/root/autodl-tmp/One-stage_Baichuan2-13B-Chat",
    revision="v2.0",
    device_map="auto",
    num_gpus = 2,
    torch_dtype=torch.bfloat16,
    trust_remote_code=True)
model.generation_config = GenerationConfig.from_pretrained("/root/autodl-tmp/One-stage_Baichuan2-13B-Chat", revision="v2.0")
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_One-stage_Baichuan2-13B-Chat:",answer0.strip().replace("\n",""))
    
    df = pd.concat([df, df2], axis=0)

  save_df_to_excel(df, "One-stage_Baichuan2-13B-Chat.xlsx", "sheet1")
