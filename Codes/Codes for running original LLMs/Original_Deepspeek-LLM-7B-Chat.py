...
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/deepseek-llm-7b-chat/", trust_remote_code=True)
model = AutoModel.from_pretrained("/root/autodl-tmp/deepseek-llm-7b-chat/",  trust_remote_code=True,top_p=0, 
  temperature=0).cuda()
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_Original_Deepspeek-LLM-7B-Chat:",answer1.strip().replace("\n",""))


    df = pd.concat([df, df2], axis=0)

  save_df_to_excel(df, "Original_Deepspeek-LLM-7B-Chat", "sheet1")
