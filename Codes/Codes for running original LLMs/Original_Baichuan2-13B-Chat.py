...
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/Baichuan2-13B-Chat",
    revision="v2.0",
    use_fast=False,
    trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained("/root/autodl-tmp/Baichuan2-13B-Chat",
    revision="v2.0",
    device_map="auto",
    num_gpus = 2,
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_Original_Baichuan2-13B-Chat:",answer0.strip().replace("\n",""))
    
    df = pd.concat([df, df2], axis=0)

  save_df_to_excel(df, "Original_Baichuan2-13B-Chat.xlsx", "sheet1")
