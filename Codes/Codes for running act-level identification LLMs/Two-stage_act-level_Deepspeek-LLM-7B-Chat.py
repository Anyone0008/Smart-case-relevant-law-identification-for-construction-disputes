...
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/Two-stage_act-level_Deepspeek-LLM-7B-Chat", trust_remote_code=True)
model = AutoModel.from_pretrained("/root/autodl-tmp/Two-stage_act-level_Deepspeek-LLM-7B-Chat",  trust_remote_code=True,top_p=0, 
  temperature=0).cuda()
...
for year in years:
  ...
  for i in range(len(Questions)):
    ...
    print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_Two-stage_act-level_Deepspeek-LLM-7B-Chat:",answer1.strip().replace("\n",""))

    df = pd.concat([df, df2], axis=0)
  df["信息2"] = df.apply(lambda row: f"{row['信息2']}请根据以上法律事实，在以下法律中推荐该案件所适用的法律具体条目，禁止胡乱编造：{row['Answer1']}", axis=1)
  save_df_to_excel(df, "Two-stage_act-level_Deepspeek-LLM-7B-Chat", "sheet1")
